...
论文看完回来了，总而言之，MapReduce牛逼。

继续看课。

谷歌的困境在于在数十 TB 的数量上快速创建索引等。

设计思想其实就是让程序员只专心于写业务代码，而不用去考虑任何和分布式有关的问题。

然后讲的都是 MapReduce 里面具体的内容了，其实和论文大差不差，因为设定上 MapReduce 这篇论文是预习，所以课程里只是简单的讲了一下它的流程。
