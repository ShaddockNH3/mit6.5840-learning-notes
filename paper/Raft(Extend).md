...
![图 1：复制状态机架构。共识算法负责管理一个复制日志，该日志包含了来自客户端的状态机指令。各个状态机处理来自日志的完全相同的指令序列，因此它们会产生相同的输出。](./img/p3-f1.png)
...
### 图 2：Raft 共识算法的浓缩摘要（不包括成员变更和日志压缩）。左上角框中的服务器行为被描述为一组独立且重复触发的规则。像 §5.2 这样的章节编号指示了特定功能在论文中讨论的位置。形式化规范 [31] 更精确地描述了该算法

这里我们将原图的内容分成了 **状态 (State)**、**RPC 接口** 和 **服务器规则 (Rules)** 三个部分，就像把原本的方框重新画好了一样哦。

#### 1. 状态 (State)

*这些是服务器里需要维护的数据变量。*

**所有服务器上的持久化状态 (Persistent state on all servers):**
*(在响应 RPC 之前必须更新到持久化存储/硬盘中)*

| 变量名             | 解释                                                    |
| :-------------- | :---------------------------------------------------- |
| **currentTerm** | 服务器看到的最新**任期号**（第一次启动时初始化为 0，单调递增）。                   |
| **votedFor**    | 在当前任期内收到选票的**候选人 ID**（如果没有则为 null）。                   |
| **log[]**       | **日志条目**；每个条目包含状态机指令，以及该条目被 Leader 接收时的任期号（第一个索引是 1）。 |

**所有服务器上的易失性状态 (Volatile state on all servers):**
*(也就是存在内存里的，重启会丢失)*

| 变量名             | 解释                                           |
| :-------------- | :------------------------------------------- |
| **commitIndex** | 已知已**提交**（committed）的最高日志条目的索引（初始化为 0，单调递增）。 |
| **lastApplied** | 已经被**应用**到状态机的最高日志条目的索引（初始化为 0，单调递增）。        |
...
| 变量名              | 解释                                                                      |
| :--------------- | :---------------------------------------------------------------------- |
| **nextIndex[]**  | 对于每一个 Follower，Leader **下一个**将要发送给它的日志条目的索引（初始化为 Leader 最新日志索引 ( +1 )）。 |
| **matchIndex[]** | 对于每一个 Follower，已知已**复制**到该服务器上的最高日志条目的索引（初始化为 0，单调递增）。                  |

#### 2. RPC 接口 (Remote Procedure Calls)

*这是服务器之间打电话交流的方式哦。*

**RequestVote RPC (请求投票)**
*由候选人 (Candidates) 调用以收集选票 (参考 §5.2)。*

* **参数 (Arguments):**

  * `term`: 候选人的任期号。
  * `candidateId`: 请求投票的候选人 ID。
  * `lastLogIndex`: 候选人最后一条日志条目的索引 (参考 §5.4)。
  * `lastLogTerm`: 候选人最后一条日志条目的任期号 (参考 §5.4)。

* **结果 (Results):**

  * `term`: 接收者的 `currentTerm`，用于让候选人更新自己。
  * `voteGranted`: 如果为 `true`，则表示候选人获得了选票。

* **接收者实现逻辑 (Receiver implementation):**

  1. 如果 ( \text{term} < \text{currentTerm} )，返回 `false` (参考 §5.1)。
  2. 如果 `votedFor` 为空 (null) 或者等于 `candidateId`，并且候选人的日志**至少**和接收者一样新 (up-to-date)，则投赞成票 (参考 §5.2, §5.4)。

**AppendEntries RPC (追加条目)**
*由 Leader 调用以复制日志条目 (参考 §5.3)；也被用作心跳 (Heartbeat, 参考 §5.2)。*

* **参数 (Arguments):**

  * `term`: Leader 的任期号。
  * `leaderId`: 用于让 Follower 能将客户端重定向给 Leader。
  * `prevLogIndex`: 紧接在新日志条目**之前**的那个日志条目的索引。
  * `prevLogTerm`: `prevLogIndex` 处条目的任期号。
  * `entries[]`: 需要存储的日志条目（如果是心跳则为空；为了效率可能会发送多个）。
  * `leaderCommit`: Leader 的 `commitIndex`。

* **结果 (Results):**

  * `term`: 接收者的 `currentTerm`，用于让 Leader 更新自己。
  * `success`: 如果 Follower 包含与 `prevLogIndex` 和 `prevLogTerm` 匹配的条目，则为 `true`。

* **接收者实现逻辑 (Receiver implementation):**

  1. 如果 ( \text{term} < \text{currentTerm} )，返回 `false` (参考 §5.1)。
  2. 如果日志中不包含在 `prevLogIndex` 处任期号匹配 `prevLogTerm` 的条目，返回 `false` (参考 §5.3)。
  3. 如果现有的条目与新条目冲突（索引相同但任期号不同），**删除**现有的该条目及其之后的所有条目 (参考 §5.3)。
  4. **追加**任何在日志中尚不存在的新条目。
  5. 如果 `leaderCommit` ( > ) `commitIndex`，将 `commitIndex` 设置为 ( \min(\text{leaderCommit}, \text{新条目最后一条的索引}) )。

#### 3. 服务器规则 (Rules for Servers)

*这是大家必须遵守的家规！*

**所有服务器 (All Servers):**

* 如果 `commitIndex` ( > ) `lastApplied`：将 `lastApplied` 加 1，并将 `log[lastApplied]` **应用**到状态机 (参考 §5.3)。
* 如果 RPC 请求或响应中包含的任期号 ( T > \text{currentTerm} )：设置 `currentTerm` ( = T )，并转换为 **Follower (跟随者)** (参考 §5.1)。

**跟随者 (Followers) (参考 §5.2):**

* 响应来自 Candidates 和 Leaders 的 RPC。
* 如果**选举超时**（election timeout）时间到了，且没有收到来自当前 Leader 的 `AppendEntries RPC`，也没有给 Candidate 投票：转换为 **Candidate (候选人)**。

**候选人 (Candidates) (参考 §5.2):**

* 当转换为 Candidate 时，开始选举：

  * 将 `currentTerm` 加 1。
  * 给自己投票 (`votedFor` = self)。
  * 重置选举计时器。
  * 向所有其他服务器发送 `RequestVote RPC`。
* 如果收到来自**大多数**服务器的选票：成为 **Leader**。
* 如果收到来自新 Leader 的 `AppendEntries RPC`：转换为 **Follower**。
* 如果选举超时：开始新一轮选举。

**领导者 (Leaders):**

* 一旦当选：向每个服务器发送初始的空 `AppendEntries RPC`（**心跳**）；在空闲期间重复发送以防止选举超时 (参考 §5.2)。
* 如果收到来自客户端的指令：将条目追加到本地日志，在条目应用到状态机后响应客户端 (参考 §5.3)。
* 如果对于某个 Follower，最后一条日志索引 ( \ge ) `nextIndex`：发送从 `nextIndex` 开始的日志条目的 `AppendEntries RPC`。

  * 如果成功：更新该 Follower 的 `nextIndex` 和 `matchIndex` (参考 §5.3)。
  * 如果因为日志不一致导致 `AppendEntries` 失败：减小 `nextIndex` 并重试 (参考 §5.3)。
* 如果存在一个 ( N )，使得 ( N > \text{commitIndex} )，且大多数的 `matchIndex[i]` ( \ge N )，并且 `log[N].term` ( == \text{currentTerm} )：设置 `commitIndex` ( = N ) (参考 §5.3, §5.4)。

### 图 3：Raft 保证这些属性在任何时刻都是成立的。章节编号指示了讨论每个属性的位置。

**选举安全性 (Election Safety)**：
在给定的任期内，最多只有一位领导者能被选出。 (§5.2)

**领导者只追加原则 (Leader Append-Only)**：
领导者永远不会覆盖或删除其日志中的条目；它只追加新的条目。 (§5.3)

**日志匹配原则 (Log Matching)**：
如果两个日志包含具有相同索引和任期号的条目，那么这两个日志在该索引之前（包括该索引）的所有条目上都是完全相同的。 (§5.3)

**领导者完整性 (Leader Completeness)**：
如果某个日志条目在给定的任期内被提交了，那么该条目将会出现在所有更高任期号的领导者的日志中。 (§5.4)

**状态机安全性 (State Machine Safety)**：
如果某个服务器已经将给定索引处的日志条目应用到了它的状态机，那么没有任何其他服务器会针对该同一索引应用不同的日志条目。 (§5.4.3)
...
![图 4：服务器状态。跟随者 (Followers) 只响应来自其他服务器的请求。如果跟随者没有收到任何通信，它就会变为候选人 (Candidate) 并发起选举。获得整个集群大多数选票的候选人将成为新的领导者 (Leader)。领导者通常会一直运作直到其发生故障。](./img/p3-f4.png)
...
![图 5：时间被划分为任期 (Terms)，每个任期都以选举开始。在选举成功后，一位领导者会管理集群直到该任期结束。有些选举会失败，这种情况下该任期结束时没有选出领导者。不同服务器可能会在不同的时间观察到任期之间的转换。](./img/p3-f5.png)
...
![图 6：日志由条目组成，这些条目是按顺序编号的。每个条目都包含其创建时的任期号（每个方框中的数字）和一个用于状态机的指令。如果将某个条目应用到状态机是安全的，那么该条目就被认为是已提交的。](./img/p3-f6.png)
...
![图 7：当最顶端的领导者上任时，跟随者的日志中可能会出现 (a–f) 场景中的任何一种。每一个方框代表一个日志条目；方框中的数字是它的任期号。一个跟随者可能会丢失条目 (a–b)，可能会有多余的未提交条目 (c–d)，或者两者都有 (e–f)。例如，场景 (f) 可能会这样发生：该服务器曾是任期 2 的领导者，向其日志中添加了几条条目，但在提交任何一条之前就崩溃了；它很快重启，成为了任期 3 的领导者，并向其日志中又添加了几条条目；在任期 2 或任期 3 的任何条目被提交之前，该服务器再次崩溃，并且在随后的几个任期内一直处于停机状态。](./img/p3-f7.png)
...
![图 8：一个时间序列，展示了为什么领导者不能利用旧任期的日志条目来确定提交状态。* 在 **(a)** 中，( S_1 ) 是领导者，并且部分复制了索引为 2 的日志条目。* 在 **(b)** 中，( S_1 ) 崩溃；( S_5 ) 获得了 ( S_3 )、( S_4 ) 和它自己的选票，当选为任期 3 的领导者，并在日志索引 2 处接受了一个不同的条目。* 在 **(c)** 中，( S_5 ) 崩溃；( S_1 ) 重启，当选为领导者，并继续复制。此时，来自任期 2 的日志条目虽然已经被复制到了大多数服务器上，但它并没有被**提交**。* 如果 ( S_1 ) 像 **(d)** 中那样崩溃，( S_5 ) 可能会（依靠 ( S_2 )、( S_3 ) 和 ( S_4 ) 的选票）当选为领导者，并用它自己任期 3 的条目**覆盖**该条目。* 然而，如果 ( S_1 ) 在崩溃之前，将来自其**当前任期**的条目复制到了大多数服务器上，如 **(e)** 所示，那么该条目就是已提交的（此时 ( S_5 ) 无法赢得选举）。此时，日志中所有之前的条目也都被提交了。](./img/p3-f8.png)
...
![图 9：如果 ( S_1 )（任期 ( T ) 的领导者）提交了一条来自其任期的新日志条目，而 ( S_5 ) 被选为了之后某个任期 ( U ) 的领导者，那么肯定至少存在一个服务器（( S_3 )），它既接受了该日志条目，同时也给 ( S_5 ) 投了票。](./img/p3-f9.png)
...
![图 10：直接从一种配置切换到另一种配置是不安全的，因为不同的服务器会在不同的时间进行切换。在这个例子中，集群从三台服务器扩展到了五台。不幸的是，存在这样一个时间点，同一个任期内可能会选出两位不同的领导者，其中一位获得了旧配置 (( C_{old} )) 中大多数的支持，而另一位获得了新配置 (( C_{new} )) 中大多数的支持。](./img/p3-f10.png)
